...
def interpret_sc4_timeline_csv(timeline_file_path, num_dust_clouds, dust_img_path='./image_files/dust1.png'):
	...
	growth_change_days, vignettes, clouds, cloud_locations = [], [], [], []
	for i in range(num_dust_clouds):
		changes_df = df[df['changes_occurred_{}'.format(i + 1)] == 1]
		growth_change_days.append(changes_df['day'].values.tolist())
	...
	for day in tqdm(range(1, num_days + 1), desc='Generating scenario 2 csv file'):
		for i in range(num_dust_clouds):
			...
			if day % clean_interval == 0:
				vignette_strengths[i] = 99.9
				growth_nums[i] = 0
				curr_probs[i] = 0
				clean_changes = 1
			elif curr_probs[i] >= incr_thresh[i] and growth_nums[i] < max_dust_growths:
		...
		line = [
			...
			vignette_strengths[0], dust_vars[0]['region'], growth_nums[0], curr_probs[0], growth_changes[0],

			dust_vars[1]['rows'], dust_vars[1]['cols'], dust_vars[1]['loc_x'], dust_vars[1]['loc_y'],
			vignette_strengths[1], dust_vars[1]['region'], growth_nums[1], curr_probs[1], growth_changes[1],

			dust_vars[2]['rows'], dust_vars[2]['cols'], dust_vars[2]['loc_x'], dust_vars[2]['loc_y'],
			vignette_strengths[2], dust_vars[2]['region'], growth_nums[2], curr_probs[2], growth_changes[2],
		]
...
def gen_scenario_4_images(
	...
	csv_data = interpret_sc4_timeline_csv(timeline_file_path, num_dust_clouds, dust_img_path)
	...
	for day in tqdm(range(1, csv_data['total_days'] + 1), desc='Generating scenario 2 images'):
		...
		for i in range(num_dust_clouds):
			if day in csv_data['clean_change_days']:
				growth_nums[i] = 0
			elif day in csv_data['growth_change_days'][i]:
...
if __name__ == '__main__':
	...
	data_dir = '/Users/nick_1/Bell_5G_Data/synth_datasets/{}/scenario_4'.format(data_partition)

	# ----- ----- ----- #
	gen_scenario_4_timeline(data_dir, num_days=days_in_timeline, clean_interval=maintenance_interval)
	gen_scenario_4_images(src_dir, data_dir)
